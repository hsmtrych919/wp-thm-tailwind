...
//カテゴリ選択のラジオボタン化 カスタム投稿除外
add_action('admin_print_footer_scripts', 'my_print_footer_scripts', 21);
function my_print_footer_scripts() {
  echo "<script type='text/javascript'>
  jQuery(document).ready(function($){
    function convertToRadioButtons(checklistID) {
      var checkLists = $(checklistID).find('li');
      checkLists.each(function(){
          var check = $(this).find('input');
          var isChecked = check.is(':checked'); // 元のチェック状態を保存
          var input = $('<input>');
          input.attr({
              type: 'radio',
              id: check.attr('id'),
          ...
          if (isChecked) {
              input.prop('checked', true); // 元のチェック状態を引き継ぐ
          }
    ...
    if ($('#post_type').val() == 'post') {
      // 通常の投稿の場合
      convertToRadioButtons('#categorychecklist');
      } else if ($('#post_type').val() == 'custom_posts') {
      // カスタム投稿 'custom_posts'の場合
      convertToRadioButtons('#arrivals_categorychecklist');
      convertToRadioButtons('#arrivals_storeschecklist');
    }
  });
  </script>";
}
