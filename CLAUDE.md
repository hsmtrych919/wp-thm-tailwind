# SCSS → Tailwind CSS 移行プロジェクト

## 現在の Phase

**Phase 5: ベンダー CSS の整理**（完了）

Phase の詳細は計画書 §7 を参照。Phase 1 の作業内容は §1 を参照。

---

## 禁止事項

1. **spacing スケールを Tailwind デフォルトに合わせるな。** `mt-4` = 32px（現状の `$space_values`）を維持する。Tailwind デフォルトの `mt-4` = 16px にしてはならない。`theme.spacing` で現状スケールを再定義する。
2. **コンポーネントクラスを Tailwind ユーティリティに分解するな。** `.button`, `.header` 等は `@layer components` でクラスごと定義する。
3. **SCSS 計算関数を削除・書き換えするな。** `g.rem()`, `g.get_vw()`, `g.get_lh()` 等はそのまま維持する。
4. **ベンダー CSS に手を入れるな。** FontAwesome, Swiper, Micromodal, scroll-hint, Ultimate Member, WP Instagram Feed は SCSS 変数→CSS 変数の差し替えのみ。ロジックやセレクタを変更しない。
5. **現在の Phase 以外のファイルを変更するな。** Phase 1 なら §1 の範囲のみ。ユーティリティ SCSS の削除（Phase 2）、PHP テンプレートの書き換え（Phase 2/3/6）等を先行してやらない。
6. **クラス名を変更したら `src/scss/_archive/_class-rename-log.md` に記録せよ。** 記録せずに次の作業に進むな。
7. **計画書に書かれていない方針判断を勝手にするな。**「モダン化」「リファクタリング」等の価値判断が必要な場合は必ずユーザーに確認する。
8. **ブレークポイント mixin、`@extend`、hover mixin を書き換えるな。** SCSS コンパイルが Tailwind 処理より先に実行されるため、`@layer` 内でそのまま動作する。`@screen md` への変換、`@extend` の展開、Tailwind バリアントへの置き換えは全て不要（計画書 §4.3 #3, #4, #7 で確定済み）。

---

## 作業ルール

1. **作業開始時に `progress.md` を読め。** 前回セッションの続きから作業を再開する。
2. **計画書を読んでから作業を開始せよ。** 該当 Phase のセクションを必ず確認する。
3. **1 Phase を完了したら `progress.md` を更新せよ。** 完了した作業・残作業・次の Phase の開始条件を記録する。
4. **SCSS 変数を CSS 変数に変換する際は `src/scss/_variable-inventory.md` のカテゴリ分類に従え。** カテゴリ A → `:root`、B-config → `tailwind.config.js`、B-body → `@layer base` body 直書き、C → `.p-form` スコープ。一括で `:root` に入れるな。
5. **リセット CSS の差分補足は `src/scss/_reset-diff-inventory.md` に従え。** destyle.css 削除時に Tailwind Preflight でカバーされない項目を `@layer base` で補足する。
6. **CSS Grid と Flex の使い分けを守れ。** ページレイアウト（旧 `.l-row` 系）→ Flex。カードグリッド（旧 `.l-grid` 系）→ CSS Grid。理由は計画書 §3.2 参照。
7. **ガターは Tailwind カスタムユーティリティを使え。** `theme.extend.padding` / `theme.extend.gap` で定義する。`@layer components` で旧クラスを維持しない。変換表は計画書 §3.4 参照。

---

## 実装指示プロンプト作成ガイド

このセクションは、**AI に実装を指示するためのプロンプトを書くときだけ**使う。
一般的な相談、調査、レビュー、報告には適用しない。

目的は 1 つだけ。

- **実装 AI に、曖昧な解釈や都合のよい完了判定をさせず、最後まで責任を持って正しく仕事を完了させること**

### 失敗パターン

実装指示プロンプトを作るときは、少なくとも次の失敗を再発させないこと。

1. 調査・設計の結論を、そのまま実装完了条件へ変換せずに流してしまう
2. 仕様、制約、非対象、完了条件を書かず、実装者の解釈に委ねてしまう
3. 作業の途中経過と完了状態を区別せず、一部だけ終わった状態を完了と誤認する
4. 確認範囲を狭く切りすぎ、成果物全体の整合確認を省く
5. 実装者の裁量で後回し、保留、黙認、自己判断を許す余地を prompt 側に残す

### 実装 prompt に必ず書くこと

#### 1. まず「何を作るか」ではなく「何を成立させるか」を書け

個別の作業内容だけで完了扱いにしてはいけない。
必ず **作業後に成果物全体がどういう状態になっているべきか** を書く。

最低限、変更対象ごとに次を明記すること。

- 実現したい状態は何か
- 満たすべき条件は何か
- 壊してはいけない前提は何か
- 完了と見なす基準は何か

実装 AI は、依頼文に書かれた作業を終えるのではなく、依頼文に書かれた状態を成立させる責任を負う形にしろ。

#### 2. スコープ、非スコープ、判断禁止事項を先に固定しろ

着手前に次を明示しろ。

- 何を変更してよいか
- 何を変更してはいけないか
- 何は今回の対象外か
- どんな判断を勝手にしてはいけないか

これがない prompt は、実装者に境界線の自己判断を許す prompt だと見なせ。

#### 3. 着手前に確認項目を定義しろ

見えている差分だけを追わせると、実装者は都合のよい範囲だけ確認して終わりやすい。
そのため prompt では、着手前に次を明示させること。

- 実装前に確認すべき前提
- 実装中に崩してはいけない条件
- 実装後に確認すべき項目
- 完了報告前に潰すべき残件

確認範囲が曖昧な prompt を作るな。境界が曖昧だと、実装者は自分に都合のよい解釈で作業を閉じる。

#### 4. 調査・設計の結論は、実行可能な指示へ展開し直せ

調査書に「統合推奨」「整理する」「寄せるべき」とあるだけでは不十分。
実装 prompt では、結論を **実行可能な作業指示** に分解すること。

悪い書き方:

- 調査書に従って実装すること

必要な書き方:

- 何をやるのか
- 何をやらないのか
- どの順で進めるのか
- どの条件を満たしたら次に進めるのか

結論の要約だけを渡すな。実装者が迷わず手を動かせる粒度まで落とし込むこと。

#### 5. 曖昧な裁量を禁止しろ

実装者に裁量を渡しすぎると、都合のよい省略、先送り、勝手な簡略化が発生する。
そのため prompt では次の原則を使うこと。

- 許可する例外は明示列挙する
- 書いていない例外は許可しない
- 「必要なら」「適宜」「判断に任せる」「可能なら」のような曖昧表現は禁止する

#### 6. 作業単位ごとに「閉じた完了チェック」を入れろ

作業単位が終わるたびに、少なくとも次を確認させること。

- その単位の目標が達成されているか
- 未完のものが残っていないか
- 次の単位へ進んでよい条件を満たしているか
- 問題がある場合に、成功扱いせず止まれているか
- 未完や保留を明示できているか

中間状態を許すなら、その単位の終わりで何が未完なのかも明示させること。

#### 7. 最終報告フォーマットにも未完検知を埋め込め

最終報告を自由形式にすると、実装者が曖昧な完了報告をしやすい。
実装 prompt には、最終報告で最低限次を言わせること。

- 実装したこと
- まだやっていないこと
- 例外として残したこと
- リスク、前提、未確認事項

この 4 つを書かせない prompt は不十分。

### 実装 prompt の必須テンプレート

実装指示プロンプトには、少なくとも次の粒度を含めること。

1. source of truth
2. 目的
3. 作業後の最終到達状態
4. 確認範囲と完了判定の条件
5. スコープと非スコープ
6. 残置を許す場合の明示リスト
7. 最終報告で列挙させる項目

### 禁止表現

次のような prompt の書き方は禁止する。

1. 「調査書に従って実装して」だけで終える
2. 「不要になったら削除」だけで、何が不要かを書かない
3. 「見えている範囲だけ直せばよい」と受け取れる書き方をする
4. 「あとは適宜対応」で止め、完了条件を書かない
5. 「build が通れば OK」とする

### このガイドの適用範囲

このガイドは、次のような実装指示 prompt に必ず適用する。

- 新機能を作る指示
- 既存実装を直す指示
- 構成や責務を変える指示
- 置換、削除、追加、改修を伴う指示

逆に、単純な調査依頼、監査依頼、レビュー依頼にはそのまま適用しない。

---

## 検証ルール

1. **`rem` / `vw` の検証は件数ではなく「クラス名と出力宣言」で確認せよ。** 単純な出現件数の比較は禁止。
2. **比較対象は原則として移行前 CSS と移行後 CSS の 2 ファイルに限定する。** 例: `css/init.css` と `css/style.css`。検証のために無関係なソースファイルや他言語ファイルへ探索を広げるな。
3. **確認単位は「`rem` / `vw` を含む既存クラスセレクタ」と「そのクラス内の宣言値」。** 次の順序で確認すること:
    - クラスが移行後 CSS に残っているか
    - 同じクラスで `rem` / `vw` を使う宣言が残っているか
    - 値の差がある場合、それが実寸を保った正規化（例: `56rem` → `3.5rem`）か、挙動変更かを判定する
4. **gutter 系の `vw` 変数は別枠で必ず直接比較する。** `--unit`, `--gutter`, `--gutter-row` の系列値が移行前後で一致していることを確認する。
5. **検証結果の記録は件数中心で書くな。** `progress.md` には「どの代表クラスを照合し、値が維持されたか」「崩れがなかったか」を優先して記録する。
6. **Phase 1 以降も同じ検証方針を使う。** 特にユーティリティ層・レイアウト層の移行では、件数の一致ではなく既存クラスの出力保持を根拠に判断する。

---

## 参照ファイル

| ファイル | 内容 | いつ読むか |
|---|---|---|
| `src/scss/_archive/tailwind-migration-plan.md` | マスター計画書（全 Phase の設計・方針・変換表） | 各 Phase 開始時に該当セクションを読む |
| `src/scss/_archive/_variable-inventory.md` | SCSS 変数のカテゴリ分類と変換先 | Phase 1 相当の変数整理内容を確認するときに参照 |
| `src/scss/_archive/_reset-diff-inventory.md` | destyle.css vs Tailwind Preflight の差分一覧 | Phase 1 相当の reset 差分確認時に参照 |
| `src/scss/_archive/_class-rename-log.md` | クラス名変更の全記録 | Phase 2/3 の変換履歴やリストラクチャのリネーム記録を確認するときに参照 |
| `src/scss/_archive/` | 旧計画書、監査、調査、実行プロンプトを含むアーカイブ資料一式 | 現行コードの経緯や過去判断を確認したい場合のみ参照 |

---

## ディレクトリ構造

```
wp-thm/                    ← Claude Code 起動ディレクトリ（git root）
├── CLAUDE.md              ← このファイル
├── progress.md            ← 進捗管理
├── *.php                  ← PHP テンプレート
├── package.json
├── webpack.config.js
└── src/
    └── scss/
        ├── style.scss             ← Sass エントリーポイント
        ├── _tailwind-base-layer.scss
        ├── tailwind-base.css      ← Tailwind v4 エントリーポイント
        ├── readme.md
        ├── global/                ← 共通 API（変数, 関数, mixin）
        ├── features/              ← 全自作 SCSS（旧 layout/ + component/ + project/）
        ├── vendor/                ← サードパーティ CSS
        └── _archive/              ← 旧計画書・棚卸し資料・退避済み SCSS
```

---

## Phase 一覧（計画書 §7 より）

| Phase | 内容 | 計画書セクション | 状態 |
|---|---|---|---|
| 1 | 基盤構築（config, base, CSS 変数, パイプライン） | §1 | 完了 |
| 2 | ユーティリティ層の移行 | §2 | 完了 |
| 3 | グリッド・レイアウト層の移行 | §3 | 完了 |
| 4 | コンポーネント・プロジェクト層の移行 | §4 | 完了 |
| 5 | ベンダー CSS の整理 | §5 | 完了 |
