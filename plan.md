...
- **`rem` / `vw` の検証は件数比較ではなく、既存クラス名とその出力宣言の照合で行う。**
- 比較対象は原則として移行前 CSS (`css/init.css`) と移行後 CSS (`css/style.css`) の 2 ファイルのみとし、無関係なソース探索に広げない。
- 照合手順は以下に固定する:
  1. `rem` / `vw` を含む既存クラスセレクタを移行前 CSS から拾う
  2. 同じクラスが移行後 CSS に残っているか確認する
  3. そのクラス内の `rem` / `vw` 宣言が残っているかを確認する
  4. 値の表現が変わっていても、実寸維持の正規化か挙動変更かを判定する
- **gutter 系 `vw` 変数 (`--unit`, `--gutter`, `--gutter-row`) は必ず移行前後で直接比較する。**
- `progress.md` に記録する際は、件数ではなく「照合した代表クラス」と「値が維持されていること」を根拠として書く。
